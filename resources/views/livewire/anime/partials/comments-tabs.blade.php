...
<ul class="flex flex-row items-start gap-8" wire:key="comments-tabs">
    <li>
        <button wire:click="setTab('comments')" wire:key="tab-comments"
            ...
            <h4 class="text-white font-normal text-2xl leading-none">Yorumlar</h4>
            <span
                class="flex items-center justify-center min-w-5 h-5 px-1.5 rounded-md bg-primary text-xs text-white/80 font-normal">
                {{ $commentsCount }}
            </span>
        </button>
    </li>
    @if($showReviews)
        <li>
            <button wire:click="setTab('reviews')" wire:key="tab-reviews"
                ...
                <h4 class="text-white font-normal text-2xl leading-none">İncelemeler</h4>
                <span
                    class="flex items-center justify-center min-w-5 h-5 px-1.5 rounded-md bg-primary text-xs text-white/80 font-normal">
                    {{ $reviewsCount }}
                </span>
