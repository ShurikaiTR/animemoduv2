            ...
            <div
                ...
                @foreach($this->days as $day)
                    <button wire:click="setDay('{{ $day->value }}')" @class([
                        ...
                        'bg-primary text-white shadow-lg shadow-primary/20 scale-105' => $this->activeDay === $day->value,
                        'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white' => $this->activeDay !== $day->value
                    ])>
                        <span class="text-xs font-bold uppercase tracking-wider opacity-70 mb-1">
                            {{ mb_substr($day->getLabel(), 0, 3) }}
                        </span>
                        <span @class(['text-lg font-bold', 'scale-110' => $this->activeDay === $day->value])>
                            {{ $day->getLabel() }}
                        </span>

                        @if($this->activeDay === $day->value)
