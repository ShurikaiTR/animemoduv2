...
@if($featuredAnimes->isNotEmpty())
    <div x-data="{
        ...
        @touchstart="handleTouchStart($event)" @touchend="handleTouchEnd($event)" role="region"
        aria-label="Öne Çıkan Animeler" {{ $attributes->merge(['class' => 'relative w-full h-128 md:h-144 lg:h-160 min-h-128 overflow-hidden group bg-black rounded-none md:rounded-3xl shadow-none md:shadow-2xl ring-0 md:ring-1 ring-white/10']) }}>
        {{-- Slides --}}
        <div class="relative w-full h-full">
            @foreach($featuredAnimes as $index => $anime)
                <x-home.hero-slide :anime="$anime" :index="$index" :activeSlide="'activeSlide'" />
            @endforeach
        </div>

        {{-- Pagination Indicators (Fixed & Aligned) --}}
        <div class="absolute inset-0 flex items-end pb-6 md:pb-9 lg:pb-11 pl-4 md:pl-12 lg:pl-16 pointer-events-none">
            <div class="z-30 flex items-center justify-start gap-2 pointer-events-auto">
