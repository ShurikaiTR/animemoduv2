...
<aside {{ $attributes->merge(['class' => 'w-64 shrink-0 sticky top-32 space-y-8']) }}>
    {{-- Search --}}
    <div class="space-y-3">
        <h3 id="sidebar-search-title" class="text-sm font-bold text-white/50 uppercase tracking-wider font-rubik">Arama</h3>
    ...
    {{-- Sort --}}
    <div class="space-y-3">
        <h3 id="sidebar-sort-title" class="text-sm font-bold text-white/50 uppercase tracking-wider font-rubik">Sıralama</h3>
        <div class="space-y-1" role="group" aria-labelledby="sidebar-sort-title">
            @foreach([
                'populer' => 'Popüler',
                'yeni' => 'En Yeni',
                'puan' => 'IMDb Puanı',
    ...
    <div class="space-y-3">
        <div class="flex items-center justify-between">
            <h3 id="sidebar-genres-title" class="text-sm font-bold text-white/50 uppercase tracking-wider font-rubik">Türler</h3>
            @if(!empty($genres))
                <button 
                    wire:click="toggleGenre('hepsi')"
                    aria-label="Tüm türleri temizle"
        ...
        <div class="space-y-1 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar" role="group" aria-labelledby="sidebar-genres-title">
            <button 
                wire:click="toggleGenre('hepsi')"
                aria-pressed="{{ empty($genres) ? 'true' : 'false' }}"
