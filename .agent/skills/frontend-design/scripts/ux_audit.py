...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law
        nav_items = len(set(re.findall(r'href=["\']([^"\']+)["\']', content)))
        if nav_items > 7 and not (filename.endswith('.blade.php') and 'hidden' in content):
            self.issues.append(f"[Hick's Law] {filename}: {nav_items} unique nav links")
        ...
        for tag in img_tags:
            if not re.search(r'alt\s*=', tag, re.IGNORECASE):
                # Clean tag for better reporting
                clean_tag = re.sub(r'\s+', ' ', tag[:50]) + '...'
                self.issues.append(f"[Accessibility] {filename}: Missing img alt text in {clean_tag}")

    def audit_directory(self, directory: str) -> None:
        extensions = {'.tsx', '.jsx', '.html', '.vue', '.svelte', '.css', '.blade.php'}
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'vendor'}]
            for file in files:
